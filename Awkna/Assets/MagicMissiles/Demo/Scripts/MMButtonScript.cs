...
public class MMButtonScript : MonoBehaviour
{
	...
	MMFireProjectile effectScript;		// A variable used to access the list of projectiles
	MMProjectileScript projectileScript;
	...
	void Start ()
	{
		effectScript = GameObject.Find("MMFireProjectile").GetComponent<MMFireProjectile>();
		getProjectileNames();
...
//		print(projectileParticleName);
	...
	public void getProjectileNames()			// Find and diplay the name of the currently selected projectile
	{
		...
		projectileScript = effectScript.projectiles[effectScript.currentProjectile].GetComponent<MMProjectileScript>();
